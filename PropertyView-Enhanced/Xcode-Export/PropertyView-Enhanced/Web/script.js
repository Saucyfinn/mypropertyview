...
function openARView() {
  if (coordinateProcessor && coordinateProcessor.getSubjectProperty()) {
    ...
    if (arCoords) {
      ...
      console.log(`Using ${arCoords.corners ? arCoords.corners.length : 'baseline'} corner points from LINZ data`);
      
      // Switch to AR tab in iOS app
      if (window.webkit?.messageHandlers?.switchToAR) {
        // Send the full AR coordinate structure
        window.webkit.messageHandlers.switchToAR.postMessage(arCoords);
      } else {
        const cornerCount = arCoords.corners ? arCoords.corners.length : 2;
        alert(`AR Ready!\\n\\nProperty: ${arCoords.metadata.subjectAppellation}\\n` + 
              `Corner Points: ${cornerCount} (automated from LINZ data)\\n` +
              `Area: ${(arCoords.metadata.area / 10000).toFixed(2)} hectares`);
      }
    } else {
...
async function requestParcels(lon, lat, r = SEARCH_RADIUS_M) {
  console.log("🎯 requestParcels called with:", lat, lon, "radius:", r);
  console.log("🔑 API Key check:", window.LINZ_API_KEY ? "Present" : "MISSING");
  console.log("🏗️ CoordinateProcessor available:", !!coordinateProcessor);
  
  currentQueryLatLng = L.latLng(lat, lon);
  setTopText("Property: <strong>finding…</strong>");

  if (window.webkit?.messageHandlers?.getParcels) {
    console.log("📱 Using native iOS message handler");
  ...
  if (coordinateProcessor) {
    try {
      console.log("📡 Starting LINZ data download...");
      ...
      console.log("📊 LINZ response:", gj?.features?.length || 0, "features");
      
      if (gj.features && gj.features.length > 0) {
        console.log("🔄 Converting for AR...");
        setTopText("Property: <strong>converting for AR…</strong>");
        const arCoords = coordinateProcessor.convertToARCoordinates(lat, lon, 0);
        console.log('✅ AR coordinates prepared:', arCoords.subjectProperty?.arPoints?.length || 0, 'points for subject property');
      }
      
      console.log("🎨 Calling renderAndCenter...");
      renderAndCenter(gj);
    } catch (e) {
