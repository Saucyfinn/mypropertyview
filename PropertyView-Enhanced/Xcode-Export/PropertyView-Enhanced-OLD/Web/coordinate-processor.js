...
class CoordinateProcessor {
    ...
    /**
     * Simplified AR baseline conversion - uses only 2 GPS points
     */
    convertToARCoordinates(originLat, originLon, originAltitude = 0) {
        ...
        console.log('Simplified AR baseline conversion started');

        try {
            // Get first two boundary points to create baseline
            const coords = this.subjectProperty.coordinates;
            if (coords.length < 2) {
                throw new Error('Subject property needs at least 2 boundary points for AR baseline');
            }

            // Use first two points as AR baseline 
            const baseline = [
                {
            ...
            // Simplified AR coordinate structure - just 2 points
            this.arCoordinates = {
                type: "arBaseline",
                origin: {
                    latitude: originLat,
                ...
                metadata: {
                    ...
                    conversionMethod: "Simplified GPS Baseline",
                    accuracy: "gps",
                    timestamp: new Date().toISOString()
                }
            };

            console.log(`Simplified AR baseline ready: 2 GPS points for subject property "${this.subjectProperty.appellation}"`);
    ...
    isARReady() {
        return !!(this.arCoordinates && (this.arCoordinates.subjectProperty || this.arCoordinates.baseline));
    }
