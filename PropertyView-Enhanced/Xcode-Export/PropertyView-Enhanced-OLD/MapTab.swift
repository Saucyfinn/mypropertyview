...
struct EnhancedWebMapView: UIViewRepresentable {
    ...
    func updateUIView(_ webView: WKWebView, context: Context) {
        if webView.url != url {
            ...
            // Check if this is a local file URL (bundled web content)
            if url.isFileURL {
                print("📁 Loading local web content with file access")
                // Load with proper file access to the Web directory
                let webDirectory = url.deletingLastPathComponent()
                webView.loadFileURL(url, allowingReadAccessTo: webDirectory)
                print("✅ LoadFileURL called with read access to: \(webDirectory)")
            } else {
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        private func handleSwitchToAR(_ body: Any) {
            print("🔄 WebView handleSwitchToAR called")
            print("📦 Message body type: \(type(of: body))")
            
            guard let data = try? JSONSerialization.data(withJSONObject: body) else {
                print("❌ Failed to serialize AR coordinate data")
            ...
            print("📄 JSON payload preview: \(preview)")
            
            // First try new simplified baseline format
            if let baselineData = try? JSONDecoder().decode(ARBaselineData.self, from: data) {
                print("✅ Successfully decoded ARBaselineData")
                let arData = convertBaselineToARCoordinates(baselineData)
                print("🔄 Calling onCoordinatesReceived with converted data")
            ...
            if let arData = try? JSONDecoder().decode(ARCoordinateData.self, from: data) {
                print("✅ Successfully decoded legacy ARCoordinateData")
                parent.onCoordinatesReceived(arData)
                return
            }
            ...
            parent.onError("Failed to parse AR coordinates. Expected baseline format with 2 GPS points.")
        }
