...
import CoreLocation
import simd
// import ARCore // Add Google ARCore Geospatial API for enhanced location anchoring

// MARK: - Enhanced Multi-Tier Positioning System

enum PositioningMethod: String, CaseIterable {
    case gps = "GPS Positioning"
    case googleGeospatial = "Google Geospatial"
    case planeDetection = "Plane Detection"
    case visualMarker = "Visual Marker"
    case compassBearing = "Compass Bearing"
    case manualInput = "Manual Input"
}

class PositioningManager {
    private weak var arView: ParcelARViewManager?
    private var currentMethod: PositioningMethod = .gps
    private var fallbackTimer: Timer?
    private var gpsTimeout: Timer?
    
    init(arView: ParcelARViewManager) {
        self.arView = arView
    }
    
    func startPositioning() {
        // Start with GPS, then cascade through fallbacks
        tryGPSPositioning()
    }
    
    private func tryGPSPositioning() {
        currentMethod = .gps
        
        // Set GPS timeout (10 seconds)
        gpsTimeout = Timer.scheduledTimer(withTimeInterval: 10.0, repeats: false) { _ in
            self.tryGoogleGeospatial()
        }
        
        // GPS positioning handled by CLLocationManager
    }
    
    private func tryGoogleGeospatial() {
        gpsTimeout?.invalidate()
        currentMethod = .googleGeospatial
        
        // Google ARCore Geospatial API integration
        // TODO: Add Google ARCore Geospatial session initialization
        print("Enhanced Positioning: Attempting Google Geospatial API")
        
        // For now, fallback to plane detection after timeout
        fallbackTimer = Timer.scheduledTimer(withTimeInterval: 8.0, repeats: false) { _ in
            self.fallbackToPlaneDetection()
        }
    }
    
    private func fallbackToPlaneDetection() {
        gpsTimeout?.invalidate()
        currentMethod = .planeDetection
        
        // Enable plane detection in ARKit
        if let configuration = arView?.sceneView.session.configuration as? ARWorldTrackingConfiguration {
            configuration.planeDetection = [.horizontal, .vertical]
            arView?.sceneView.session.run(configuration)
        }
        
        // Fallback timer for plane detection (15 seconds)
        fallbackTimer = Timer.scheduledTimer(withTimeInterval: 15.0, repeats: false) { _ in
            self.fallbackToVisualMarker()
        }
    }
    
    private func fallbackToVisualMarker() {
        fallbackTimer?.invalidate()
        currentMethod = .visualMarker
        
        // Fallback timer for visual marker (20 seconds)
        fallbackTimer = Timer.scheduledTimer(withTimeInterval: 20.0, repeats: false) { _ in
            self.fallbackToCompassBearing()
        }
    }
    
    private func fallbackToCompassBearing() {
        fallbackTimer?.invalidate()
        currentMethod = .compassBearing
        
        // Final fallback timer (30 seconds)
        fallbackTimer = Timer.scheduledTimer(withTimeInterval: 30.0, repeats: false) { _ in
            self.fallbackToManualInput()
        }
    }
    
    private func fallbackToManualInput() {
        fallbackTimer?.invalidate()
        currentMethod = .manualInput
        
        // Show manual input dialog
        arView?.showManualInputDialog()
    }
    
    func onGPSSuccess(coordinate: CLLocationCoordinate2D) {
        if currentMethod == .gps {
            gpsTimeout?.invalidate()
            arView?.onGPSPositioned(coordinate: coordinate)
        }
    }
    
    func onPlaneDetected(anchor: ARAnchor) {
        if currentMethod == .planeDetection {
            fallbackTimer?.invalidate()
            arView?.onPlanePositioned(anchor: anchor)
        }
    }
...
public final class ParcelARViewManager: UIViewController, ARSCNViewDelegate, CLLocationManagerDelegate {
    ...
    // Views
    internal let sceneView = ARSCNView(frame: .zero)
    ...
    // Enhanced Multi-Tier Positioning System
    private var positioningManager: PositioningManager?
    private var currentFallbackMethod: PositioningMethod = .gps
    private var positioningStatusLabel: UILabel?
    ...
    public func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        print("[INFO] GPS failed, starting fallback positioning:", error.localizedDescription)
        if !hasRunSessionOnce {
            hasRunSessionOnce = true
            startARSessionWithFallback()
        }
    }
    
    // Enhanced AR session with fallback positioning
    private func startARSessionWithFallback() {
        print("[INFO] Starting AR session with enhanced fallback positioning")
        
        let config = ARWorldTrackingConfiguration()
        config.worldAlignment = .gravityAndHeading
        config.planeDetection = [.horizontal, .vertical] // Enable plane detection for fallback
        
        sceneView.session.run(config, options: [])
        
        // Use Wellington fallback with enhanced positioning
        let fallbackCoord = CLLocationCoordinate2D(latitude: -41.288889, longitude: 174.777222)
        sessionRefCoord = fallbackCoord
        deviceCoord = fallbackCoord
        
        // Fetch parcels at fallback location
        fetchParcels(around: fallbackCoord)
    }
    ...
    private func redrawParcelsGroup() {
        ...
        // SUBJECT: barely visible fill + clear outline per user request
        let subj = buildPolygonNode(
            rings: subject,
            ref: ref,
            fill: UIColor.systemCyan, // Let buildPolygonNode handle alpha reduction
...
private func buildPolygonNode(rings: [[CLLocationCoordinate2D]],
    ...
    for ring in rings {
        ...
        path.move(to: CGPoint(x: CGFloat(p0.x), y: CGFloat(p0.z))) // p0.z is now already negative for north
        for pt in ring.dropFirst() {
            let v = enuFrom(lat: pt.latitude, lon: pt.longitude, refLat: ref.latitude, refLon: ref.longitude)
            path.addLine(to: CGPoint(x: CGFloat(v.x), y: CGFloat(v.z))) // v.z is now already negative 
        }
        path.close()
    }

    // Barely visible translucent fill as requested by user
    let fillShape = SCNShape(path: path, extrusionDepth: 0.02)
    let fillMat = SCNMaterial()
    fillMat.diffuse.contents = fill.withAlphaComponent(0.06) // Barely visible per user request
    fillMat.lightingModel = .constant // Prevent lighting glare
    fillMat.transparency = 0.94 // Almost transparent
    fillMat.blendMode = .alpha
    fillMat.isDoubleSided = true
    fillMat.writesToDepthBuffer = false // Optional: prevent depth conflicts
...
// MARK: - Math / Helpers
/// WGS84 -> ENU (East, North, Up) around refLat/refLon - FIXED coordinate system for ARKit
private func enuFrom(lat: Double, lon: Double, refLat: Double, refLon: Double) -> simd_float3 {
    ...
    // Match JavaScript coordinate system exactly
    let east  = R * cosLat * dLon  // East-West distance (matches JS)
    let north = R * dLat           // North-South distance
    let up    = 0.0                // Ground level
    
    // CRITICAL FIX: Use -north for Z to match ARKit convention (negative Z = forward/north)
...
// MARK: - Enhanced Positioning System Implementation

extension ParcelARViewManager {
    
    func setupEnhancedPositioning() {
        positioningManager = PositioningManager(arView: self)
    }
    
    func onGPSPositioned(coordinate: CLLocationCoordinate2D) {
        deviceCoord = coordinate
        sessionRefCoord = coordinate
        DispatchQueue.main.async {
            self.refetchIfNeeded()
        }
    }
    
    func onPlanePositioned(anchor: ARAnchor) {
        // Position boundaries relative to detected plane
        if let planeAnchor = anchor as? ARPlaneAnchor {
            positionBoundariesOnPlane(planeAnchor)
        }
    }
    
    private func positionBoundariesOnPlane(_ planeAnchor: ARPlaneAnchor) {
        // Use the plane as reference and position boundaries relative to it
        let fallbackCoord = CLLocationCoordinate2D(latitude: -41.288889, longitude: 174.777222) // Wellington fallback
        
        // Use plane positioning for boundary display
        deviceCoord = fallbackCoord
        sessionRefCoord = fallbackCoord
        refetchIfNeeded()
    }
    ...
    func showManualInputDialog() {
        let alert = UIAlertController(title: "Manual Coordinates", 
                                      message: "Enter property coordinates manually", 
                                      preferredStyle: .alert)
        
        alert.addTextField { textField in
            textField.placeholder = "Latitude (e.g., -41.288889)"
            textField.keyboardType = .decimalPad
        }
        
        alert.addTextField { textField in
            textField.placeholder = "Longitude (e.g., 174.777222)"
            textField.keyboardType = .decimalPad
        }
        
        alert.addAction(UIAlertAction(title: "Use Default Location", style: .default) { _ in
            let fallbackCoord = CLLocationCoordinate2D(latitude: -41.288889, longitude: 174.777222)
            self.onManualCoordinatesEntered(coordinate: fallbackCoord)
        })
        
        alert.addAction(UIAlertAction(title: "Enter Coordinates", style: .default) { _ in
            guard let latText = alert.textFields?[0].text,
                ...
                self.showManualInputDialog() // Try again
                return
            }
            
            let coordinate = CLLocationCoordinate2D(latitude: lat, longitude: lon)
            self.onManualCoordinatesEntered(coordinate: coordinate)
        })
        
        present(alert, animated: true)
    }
    
    private func onManualCoordinatesEntered(coordinate: CLLocationCoordinate2D) {
        deviceCoord = coordinate
        sessionRefCoord = coordinate
        refetchIfNeeded()
    }
